...
Item {
  ...
  // Stem Mode properties
  AppProperty { id: stemVolume1; path: "app.traktor.decks." + deckIdx + ".stems.1.volume" }
  AppProperty { id: stemVolume2; path: "app.traktor.decks." + deckIdx + ".stems.2.volume" }
  AppProperty { id: stemVolume3; path: "app.traktor.decks." + deckIdx + ".stems.3.volume" }
  AppProperty { id: stemVolume4; path: "app.traktor.decks." + deckIdx + ".stems.4.volume" }
  AppProperty { id: stemMute1; path: "app.traktor.decks." + deckIdx + ".stems.1.muted" }
  AppProperty { id: stemMute2; path: "app.traktor.decks." + deckIdx + ".stems.2.muted" }
  AppProperty { id: stemMute3; path: "app.traktor.decks." + deckIdx + ".stems.3.muted" }
  AppProperty { id: stemMute4; path: "app.traktor.decks." + deckIdx + ".stems.4.muted" }

  MappingProperty { id: mixerShiftActiveProp; path: screen.propertiesPath + ".mixer_shift_active" }
  property alias mixerShiftActive: mixerShiftActiveProp.value

  AppProperty { id: resultingKey; path: "app.traktor.decks." + deckIdx + ".track.key.resulting.quantized" }
      ...
      case FXSectionLayer.mixer:
      {
        if (mixerShiftActive)
        {
          switch(knob)
          {
            ...
            case 4: return (keyAdjust.value < 0 ? "" : "+") + (keyAdjust.value * 12).toFixed(0);
          }
        }
        else
        {
          switch(knob)
          {
            case 1: return (100.0 * stemVolume1.value).toFixed() + (stemMute1.value ? " M" : "");
            case 2: return (100.0 * stemVolume2.value).toFixed() + (stemMute2.value ? " M" : "");
            case 3: return (100.0 * stemVolume3.value).toFixed() + (stemMute3.value ? " M" : "");
            case 4: return (100.0 * stemVolume4.value).toFixed() + (stemMute4.value ? " M" : "");
          }
        }
